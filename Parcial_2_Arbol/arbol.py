...
def inorden_con_datos(arbol):
    if(arbol is not None):
        inorden_con_datos(arbol['izq'])
        print(arbol['info'],arbol['datos'])
        inorden_con_datos(arbol['der'])
...
def inorden_empieza_con(arbol, valor):
    if(arbol is not None):
        ...
        if arbol['info'].upper().startswith(valor.upper()):
            print(arbol['info'])
        inorden_empieza_con(arbol['der'], valor)

def inorden_por_coincidencia(arbol, valor):
    if(arbol is not None):
        inorden_por_coincidencia(arbol['izq'], valor)
        if valor.upper() in arbol['info'].upper():
            print(arbol['info'])
        inorden_por_coincidencia(arbol['der'], valor)
