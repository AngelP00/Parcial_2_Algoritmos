...
class Grafo():
    ...
    def contar_maravillas2(self):
        paises = {}
        aux = self.__inicio
        while aux is not None:
            
            paises_de_la_maravilla = aux.datos['pais'].split(',')
            #print('paises_de_la_maravilla:',paises_de_la_maravilla)
            for p in paises_de_la_maravilla:
                #print('p:',p)
                if p not in paises:
                    #print('-')
                    paises[p] = {'arquitectonica_moderna': 0, 'natural': 0}

                if aux.datos['tipo'] == 'natural':
                    paises[p]['natural'] += 1
                else:
                    paises[p]['arquitectonica_moderna'] += 1
            aux = aux.sig

        return paises
    
    def mostrar_todos_los_personajes_con_sus_adyacentes(self):
        aux = self.__inicio
        while aux is not None:
            #print(aux)
            #print(aux.adyacentes)
            #for adyacente in aux.adyacentes:
            #print('adyacentes de:',aux.info)
            print(aux.info,'compartio peliculas con:')
            #aux = self.__inicio
            adyacente = aux.adyacentes.get_inicio()
            while adyacente is not None:
                #aristas.arribo([aux.info, adyacente.info], adyacente.peso)
                print('    ',adyacente.info,'en',adyacente.peso,'peliculas')
    ...
    def cuales_personajes_comparten_con_otro_dos_episodios_o_mas(self):
        aux = self.__inicio
        while aux is not None:
            print(aux.info,'compartio dos peliculas o mas con:')
            adyacente = aux.adyacentes.get_inicio()
            while adyacente is not None:
                if(adyacente.peso>=2):
                    print('    ',adyacente.info,'en',adyacente.peso,'peliculas')
    ...
    def personaje_que_a_compartido_episodio_con_la_mayor_cantidad_del_resto_de_los_personajes(self):
        aux = self.__inicio

        personaje_con_mayor_capitulos_compartidos = ''
        cantidad_del_mayor = 0
        while aux is not None:
            #print(aux.info,'compartio dos peliculas o mas con:')
            adyacente = aux.adyacentes.get_inicio()
            cant_de_este_personaje = 0
            while adyacente is not None:
                cant_de_este_personaje+=1
                #print('    ',adyacente.info,'en',adyacente.peso,'peliculas')
                adyacente = adyacente.sig
            #print(cant_de_este_personaje)
            if(cant_de_este_personaje == cantidad_del_mayor):
                personaje_con_mayor_capitulos_compartidos = personaje_con_mayor_capitulos_compartidos+','+aux.info
                cantidad_del_mayor = cant_de_este_personaje
            elif(cant_de_este_personaje > cantidad_del_mayor):
                personaje_con_mayor_capitulos_compartidos = aux.info
                cantidad_del_mayor = cant_de_este_personaje

            aux = aux.sig
        return personaje_con_mayor_capitulos_compartidos, cantidad_del_mayor
